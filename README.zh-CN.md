...
- 桌面转换流程现在把暂停、继续和取消放在主操作区：开始转换后主按钮会变成蓝色“暂停”，点击暂停后会立刻变成灰色“播放键 + 继续”，取消按钮会直接显示在下方。
- PDF 转换现在会把真实页面 DPI 传给 Node 高保真排版引擎，避免高保真输出中文字框过大、文字跑出页面。
- 带文字层的 PDF 会优先使用 PDF 原生文字坐标和内容，只有扫描件才回退 OCR，减少 PDF 文字被识别成碎词或乱码。
...
- 暂停 / 继续已经通过 worker 控制通道验证；取消已验证会返回 `转换已取消。`，不会伪装成成功结果。
- `test/Istanbul.pdf` 第 1 页已用 200 DPI、PDF 原生文字提取和 Node 高保真输出复测，生成的 PPTX 页面尺寸为正确的 `13.33 x 7.5 in`。
